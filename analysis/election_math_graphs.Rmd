...
rm(list = ls())
library(pacman)
p_load(foreign, haven, ggplot2, broom, plm, estimatr, gsynth, reshape2, zoo, stargazer, here, modelsummary, interflex, sandwich, multcomp, lmtest, kableExtra, texreg, fastDummies, ggdag, AER ,stats, tidyr, cowplot, stringr, knitr, scales, dplyr, viridis, ggrepel)
...
ggplot(data, aes(x = Demographic, y = Value, fill = Calculation_Method)) +
  ...
  scale_fill_manual(name = "", values = c("RCVD" = colors[1], "LDD" = colors[6])) +
  geom_text(
    aes(label = Calculation_Method, y = Value + sign(Value) * max(abs(Value)) * 0.05),
  ...
  labs(
    ...
    subtitle = "Comparing RCVD vs LDD Calculation",
    x = "",
    y = "Total Votes"
  ) +
  theme_bw() +
...
data_components <- data.frame(
  ...
  Calculation_Method = rep(c("LDD", "RCVD"), times = 2),
  Rate = c(
    # Non-White
  ...
  Residual = c(
    # Non-White (LDD has residual, RCV has 0)
...
# Calculate the 'Total Change' for LDD and RCV based on components
data_components <- data_components %>%
  rowwise() %>%
  mutate(Calculated_Total = sum(Rate, Composition, Volume, Residual)) %>%
  ungroup()

# Calculate total components across all demographics for LDD and RCV
...
ggplot(final_data_simple, aes(x = Calculation_Method, y = Value, fill = Component)) +
  ...
  labs(
    ...
    subtitle = "Comparing LDD vs RCVD Approach",
    x = "",
...
data_components <- data.frame(
  ...
  Calculation_Method = rep(c("LDD", "RCVD"), times = 5),
  Rate = c(
    # Black
  ...
  Residual = c(
    # Black (LDD has residual, RCV has 0)
...
ggplot(final_data_rate_mix, aes(x = Calculation_Method, y = Value, fill = Component)) +
  ...
  labs(
    title = "2020 vs 2020 Hypothetical Vote Change Decomposition",
    subtitle = "Comparing LDD vs RCVD Components",
    x = "",
    y = "Vote Change for Democratic Candidate"
  ) +
  scale_y_continuous(labels = scales::comma) +
...
ggplot(data, aes(x = Demographic, y = Value, fill = Calculation_Method)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.75), width = 0.7) +
  geom_hline(yintercept = 0, linewidth = 0.5, color = "black", linetype = "dashed") +
  scale_fill_manual(name = "Method", values = c("RCVD" = colors[1], "LDD" = colors[6])) +
  geom_text(
    aes(label = Calculation_Method, y = Value + sign(Value) * max(abs(Value)) * 0.05),
    position = position_dodge(width = 0.75),
    size = 3.5,
  ...
  labs(
    ...
    subtitle = "Comparing RCV vs LDD Calculation",
    x = "",
    y = "Total Votes"
  ) +
  scale_y_continuous(labels = scales::comma,
...
p_main <- ggplot(main_plot_data, aes(x = Component, y = Value, fill = Data_Source)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.75), width = 0.7) +
  scale_fill_manual(name = "", values = c("ANES" = colors[1], "PEW" = colors[6])) +
  #geom_text(
...
p_residual <- ggplot(residual_data, aes(x = Data_Source, y = Value, fill = Data_Source)) +
  ...
  #) +
  coord_flip() +
  scale_fill_manual(name = "Data Source", values = c("ANES" = colors[1], "PEW" = colors[6])) +

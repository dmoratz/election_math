...
```{r}
# Load margin and mix tables for simulated data

# 2x2 table
simple_2016 <- read.csv("simulated_2016_simple.csv")
simple_2020 <- read.csv("simulated_2020_simple.csv")
...
```{r}
# Only rate Changes
# create new 2020 rates, lowers rate of dem votes by 10%
data_2020_rate_only <- data_2020_updated
data_2020_rate_only$mean_dem <- data_2020_rate_only$mean_dem*.9
margin_mix_2020_rate_only <- create_margin_mix_table(data_2020_rate_only)

# Only mix Changes
# create new 2020 mix, shifts all non-white down 56%, into white voters
data_2020_mix_only <- data_2020_updated
data_2020_mix_only$draw <- data_2020_updated$draw*.44
data_2020_mix_only[4,"draw"] <- sum(data_2020_updated$draw)-sum(data_2020_mix_only[c(1:3,5),"draw"])
margin_mix_2020_mix_only <- create_margin_mix_table(data_2020_mix_only)

# Both mix and rate changes
# creates new 2020 results, shifts all non-white voters down 20%,
# lowers rate of dem votes by 7%
data_2020_rate_mix <- data_2020_updated
data_2020_rate_mix$mean_dem <- data_2020_rate_mix$mean_dem*.93
data_2020_rate_mix$draw <- data_2020_updated$draw*.8
data_2020_rate_mix[4,"draw"] <- sum(data_2020_updated$draw)-sum(data_2020_rate_mix[c(1:3,5),"draw"])
margin_mix_2020_rate_mix <- create_margin_mix_table(data_2020_rate_mix)
...
# Apply to Pew 2024
derivative_pew_2024 <- derivative_analysis(pew_margin_2020, pew_margin_2024)
derivative_pew_1624 <- derivative_analysis(pew_margin_2016, pew_margin_2024)

# Apply to simulated data
derivative_simple_sim <- derivative_analysis(simple_2016, simple_2020)
derivative_rate_only <- derivative_analysis(margin_mix_2020, margin_mix_2020_rate_only)
derivative_mix_only <- derivative_analysis(margin_mix_2020, margin_mix_2020_mix_only)
derivative_rate_mix <- derivative_analysis(margin_mix_2020, margin_mix_2020_rate_mix)
...
# Apply to simulated data
rcv_simple_sim <- rcv_analysis(simple_2016, simple_2020)
rcv_rate_only <- rcv_analysis(margin_mix_2020, margin_mix_2020_rate_only)
rcv_mix_only <- rcv_analysis(margin_mix_2020, margin_mix_2020_mix_only)
rcv_rate_mix <- rcv_analysis(margin_mix_2020, margin_mix_2020_rate_mix)

```

```{r}
# RCV Alternative Order Analysis
rcv_analysis_alt <- function(data1, data2) {
  # Calculate B/W (Dem vs Repub across 2 years) Table ---
  # This part directly calculates the differences between the 2nd election and 
  # the first election data.
  b_w_df <<- data2 %>%
  ...
  final_df <- b_w_df %>%
    mutate(
      ...
      # Rate Variance: 
      # Change in `Trad_Mar` * Total from 1st election * `Trad_Mix` from 1st election
      Rate = (data2$Trad_Mar - data1$Trad_Mar) * sum(data1$Total) * data2$Trad_Mix,
...
rcv_simple_sim_alt <- rcv_analysis_alt(simple_2016, simple_2020)
```
...
data_to_write <- list(derivative_grimmer_data = derivative_grimmer_data, 
                      ...
                      derivative_pew_2024 = derivative_pew_2024,
                      derivative_pew_1624 = derivative_pew_1624,
                      derivative_simple_sim = derivative_simple_sim,
                      derivative_rate_only = derivative_rate_only,
                      derivative_mix_only = derivative_mix_only,
                      derivative_rate_mix = derivative_rate_mix,
                      rcv_pew_data_16v20 = rcv_pew_data_16v20,
                      rcv_pew_data_20v24 = rcv_pew_data_20v24,
                      rcv_pew_data_16v24 = rcv_pew_data_16v24,
                      rcv_simple_sim = rcv_simple_sim,
                      rcv_simple_sim_alt = rcv_simple_sim_alt,
                      rcv_rate_only = rcv_rate_only,
                      rcv_mix_only = rcv_mix_only,
                      rcv_rate_mix = rcv_rate_mix)

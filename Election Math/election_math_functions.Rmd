...
rm(list = ls())
library(pacman)
p_load(tidyverse, foreign, haven, ggplot2, broom, plyr, dplyr, DataCombine, plm, estimatr, gsynth, reshape2, zoo, stargazer, did, here, etwfe, modelsummary, interflex, sandwich, multcomp, lmtest, kableExtra, texreg, fastDummies, ggdag, AER, MatchIt, cobalt, MCMCpack,coda,stats, tidyr, cowplot, blocs)
...
data_2016 <- vb_discrete(vbdf_data[which(vbdf_data_recent$year %in% "2016"),], indep = "race", dv_turnout = "voted", dv_vote3 = "vote_pres3", boot_iters = 100, weight = "weight") %>%
  group_by(race) %>%
...
data_2020 <- vb_discrete(vbdf_data[which(vbdf_data_recent$year %in% "2020"),], indep = "race", dv_turnout = "voted", dv_vote3 = "vote_pres3", boot_iters = 100, weight = "weight") %>%
  group_by(race) %>%
  summarise(mean_prob = mean(prob),
            mean_turnout = mean(pr_turnout),
            mean_rep = mean(pr_voterep),
...
# Apply to Pew Data
rcv_pew_data <- rcv_analysis(pew_margin_2016, pew_margin_2020)

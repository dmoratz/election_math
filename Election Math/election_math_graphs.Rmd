...
ggplot(
    ...
    data = data.frame(x = c(2.01, 10)),
    mapping = aes(x = x)
) +
    # 2. Add Perpendicular Segments to the Axes
    geom_segment(
        aes(x = x0, y = 0, xend = x0, yend = f_x0),
        linetype = "dotted", color = "grey50", linewidth = 0.5
    ) +
    geom_segment(
        aes(x = 0, y = f_x0, xend = x0, yend = f_x0),
        linetype = "dotted", color = "grey50", linewidth = 0.5
    ) +
    ...
    # 5. DIRECT LINE LABELING (using geom_label for clarity on the graph)
    
    # Label for the log line ("function of interest")
    geom_label(
        # Position the label on the log function curve (e.g., at x=8)
        aes(x = 6, y = log_func(6)),
    ...
    # Label for the dashed line ("FO Taylor Approximation")
    geom_label(
        # Position the label along the dashed line (e.g., at x=8)
    ...
    # 8. Ensure the plot limits are consistent
    coord_cartesian(xlim = c(0, 7), ylim = c(0, 7)) +

    # 9. Use a clean theme and remove gridlines
    theme_bw() +
    theme(
...
data_components <- data.frame(
  ...
  Calculation_Method = rep(c("Traditional", "RCVD"), times = 2),
  Rate = c(
    # Non-White
  ...
  Residual = c(
    # Non-White (Traditional has residual, RCV has 0)
...
ggplot(final_data_simple, aes(x = Calculation_Method, y = Value, fill = Component)) +
  ...
  labs(
    ...
    subtitle = "Comparing Traditional vs RCVD Approach",
...
p_residual <- ggplot(residual_data, aes(x = Data_Source, y = Value, fill = Data_Source)) +
  ...
  labs(
    title = "Total Source Residuals vs True Results - All Voting Groups", # Title for the residual chart
    subtitle = "",
    x = "", # No x-axis label

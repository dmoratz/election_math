...
rm(list = ls())
library(pacman)
p_load(foreign, haven, ggplot2, broom, DataCombine, plm, estimatr, gsynth, reshape2, zoo, stargazer, here, modelsummary, interflex, sandwich, multcomp, lmtest, kableExtra, texreg, fastDummies, ggdag, AER ,stats, tidyr, cowplot, stringr, knitr, scales, dplyr)
...
rcv_pew_2020 <- election_analysis_data$rcv_pew_data_2020
rcv_pew_2024 <- election_analysis_data$rcv_pew_data_2024
...
vote_margin_diff_2024 <- vote_margin_2024 - vote_margin_2024
...
data_hispanic <- data.frame(
  ...
  Data_Source = c("ANES", "PEW",
                         ...
                         "ANES", "PEW",
                         "ANES", "PEW"),
  Value = c(rcv_grimmer$Rate[2], rcv_pew_2020$Rate[3],    # Rate
            rcv_grimmer$Mix[2], rcv_pew_2020$Mix[3],    # Composition
            rcv_grimmer$Volume[2], rcv_pew_2020$Volume[3],    # Volume
            (sum(rcv_grimmer$Calc_Mar_Chng)-vote_margin_diff_1620), (sum(rcv_pew_2020$Calc_Mar_Chng)-vote_margin_diff_1620))     # Residual
)
...
# Calculate the 'Total Change' for Traditional and RCV based on components
data_components_2024 <- rcv_pew_2024 %>%
...
# To save the plot (optional)
ggsave("rcv_stacked_bar_chart.png", width = 8, height = 7, dpi = 300)
```
...
print(display_clean(rcv_pew_2024))
```
